                ...
                <div class="hidden sm:block 	  ">
                    <nav class="flex space-x-4 justify-end" aria-label="Tabs">
                        <Link v-for="tab in sections" :key="tab.name" :href="tab['href']"
                              :class="[tab['current'] ? 'bg-indigo-100 text-indigo-700' : 'text-gray-500 hover:text-gray-700', 'px-3 py-0 font-medium text-sm rounded-md']" :aria-current="tab['current'] ? 'page' : undefined">
                            {{ tab.name }}
                        </Link>
...
export default {
    props     : ['breadcrumbs', 'buttons', 'module', 'actionIcons'],
    components: {
        ChevronLeftIcon,
