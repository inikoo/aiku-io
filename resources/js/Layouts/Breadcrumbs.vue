        ...
        <ol role="list" class="flex items-center space-x-2">
            ...
            <li v-for="breadcrumb in  breadcrumbs" :key="breadcrumb.name">
                <div class="flex items-center">
                    ...
                    <Link :href="route(breadcrumb.route,breadcrumb['routeParameters'])" class="ml-2 text-sm font-medium text-gray-500 hover:text-gray-700" :aria-current="breadcrumb.current ? 'page' : undefined">{{ breadcrumb.name }}</Link>
                    <span class="ml-1 text-sm font-medium text-gray-400 " v-if="breadcrumb.suffix">{{breadcrumb.suffix}}</span>
                </div>
            </li>
        </ol>
    </nav>
