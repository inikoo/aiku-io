    ...
    <div class="border-b border-gray-200 ml-2 ">
        <span v-for="item in items" :key="item.name">
            <span v-if="item.type==='modelOptions'" :class="[isCurrentRouteBase(item.module)  ? 'text-gray-700' : 'text-gray-400 group-hover:text-gray-500', 'mr-4  inline-block']">
                ...
                <TopMenuLink
                    v-if=" (currentModels[item.module]  && !isCurrentRouteBase(item.module+'s') ) "
                    :href="route(item.indexRoute)" :icon="['fal', 'bars']"/>
                <TopMenuLink
                    v-if="(!isCurrentRouteBase(item.module+'s') && currentModels[item.module]  )"
                    :href="route(item.route,currentModels[item.module])" :icon="item['icon']" :label="item['code']" :isCurrent="isCurrentRouteBase(item.module)"/>
>{{currentModels[item.module]}}
                ...
                :label="item.options[
                    currentModels[item.module]]
                    ['code']" :isCurrent="isCurrentRouteBase(item.module)"/>
            ...
            <span v-else-if="item.type==='modelIndex'" :class="'mr-2'">
                ...
                <TopMenuLink v-if="(isCurrentRouteBase(item.module) || !currentModels[item.module.slice(0, -1)]  )" :href="route(item.route)" :icon="item['icon']" :label="item['code']" :isCurrent="isCurrentRouteBase(item.module)"/>
            </span>
